            ...
            `cat > commands.json <<EOF
{
  "commands": [
    ...
    "SECRET=$(aws secretsmanager get-secret-value --secret-id HealthAI-GrafanaCloud --region us-east-1 --query SecretString --output text)",
    "export LOKI_URL=$(echo \\"$SECRET\\" | python3 -c \\"import sys,json; print(json.load(sys.stdin)['GRAFANA_LOKI_URL'])\\")",
    "export LOKI_INSTANCE_ID=$(echo \\"$SECRET\\" | python3 -c \\"import sys,json; print(json.load(sys.stdin)['GRAFANA_LOKI_INSTANCE_ID'])\\")",
    "export LOKI_API_KEY=$(echo \\"$SECRET\\" | python3 -c \\"import sys,json; print(json.load(sys.stdin)['GRAFANA_API_KEY'])\\")",
    "docker-compose down",
